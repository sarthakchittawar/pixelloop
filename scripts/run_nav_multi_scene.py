...
def run_for_goal(scene_name, scene_dir, idx, goal, cfg, config_path, run_nav_path, device_id=0, edge_culling_mode="EMST_SINGLE", node_culling_mode="NONE", node_culling_factor=10, suffix="noLC"):
    ...
    episode_path = scene_dir
    costmap_base_dir = os.path.dirname(scene_dir)  # Adjust if costmaps are in a different directory
    ...
    cmd = [
        ...
        f"costmap_base_dir={costmap_base_dir}",
        f"costmap_filename={costmap_filename}",
        f"costmap_dirname={cfg.scenes.get('costmap_dirname', 'topo_map_outputs')}",
        f"device={'cuda' if device_id >=0 else 'cpu'}",
        f"results_dirpath=/scratch/sarthakc/mast3rnav/sg_habitat/output_new/multi_scene_runs_{suffix}/{scene_name}/goalImg{goal_img_idx}",
...
def main(cfg: DictConfig):
    ...
    for scene_dir in scene_list:
        ...
        for idx, goal in enumerate(goals):
            tasks.append((scene_name, scene_dir, idx, goal, cfg, cfg.get("config_path", "configs/config.yaml"), cfg.get("run_nav_path", "run_nav.py")))

    for i, task in enumerate(tasks):
        device_id = AVAILABLE_GPUS[i % len(AVAILABLE_GPUS)]
        tasks[i] = task + (device_id, edge_culling_mode, node_culling_mode, node_culling_factor, suffix)
